...
| `infos.enable_metrics_route` | `true` if  `--enable-metrics-route`/`MEILI_ENABLE_METRICS_ROUTE` is specified at launch, otherwise `false` | `false` | Every Hour |
...
| infos.enable_metrics_route | `true` if  `--enable-metrics-route`/`MEILI_ENABLE_METRICS_ROUTE` is specified at launch, otherwise `false` | `false` |
