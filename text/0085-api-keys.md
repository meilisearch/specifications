...
| expiresAt   | string  | Represent the expiration date and time as `RFC 3339` format. |
...
| expiresAt   | string  | Required | The expiration date and time as `RFC 3339` format. Sending only the date part e.g `2021-12-01` leads to having an `expiresAt` value set to `2021-12-01T00:00:00`. **Default**: `No Default` |
...
- `expiresAt` is mandatory and must be a valid `RFC 3339` datetime in the future.
