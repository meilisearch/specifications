...
- API keys management is restricted to the master key holder.
...
##### 3.2.4.3. `GET` - `/keys/:uid`

Fetch a specific API key of a Meilisearch instance from it's `uid` field.
...
|-------------|---------|----------|--------------------------------|
| uid         | string  | Optional | A unique identifier represented by uuid v4. Specified at creation or generated by Meilisearch if ommited. |
...
An `uid` representing an uuid v4 is generated if not specified at creation by the user.
