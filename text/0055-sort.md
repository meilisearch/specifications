...
- `sort` ranking rule allows developers to adjust the sorting behavior on the selection of search results.

### II. Motivation

According to our user feedback, the lack of a sorting feature is mentioned as one of the biggest deal-breakers for choosing MeiliSearch as a search engine. A search engine must be able to offer this feature, especially for e-commerce use. Moreover, competitors all offer it. Users today must find workarounds that take time to develop and maintain to be able to sort search results.
...
- 💡 Sending an inexistent field WON'T throw an error.
...
- 💡 Sending other than string value as array item results in a 400 bad request - **invalid_request_error**.
- 💡 Sending an inexistent field will not throw an error.
...
#### **As an End-User, I want specify a `sort` parameter at search time so that I can sort search result in ascending/descending order from document attributes, whether they are `numeric` or `string`.**
...
💡 Sending a value not set in `sortableAttribute` will lead to a 400 Bad Request.

```
{
    "message": "invalid sort, the fields `attributeName` are not set as sortable",
...
💡 Sending a wrong formatted value will lead to a 400 Bad Request. The valid format is `attribute_name:asc|desc`.
...
- ℹ️ The position of the sort ranking rule is significant. The higher it is, the more critical it will be. The lower it is, the less important it will be and the other rules will take priority. It allows the developer to adjust the behavior between completeness and relevancy with the sort criterion. Knowing this, if `sort`ranking rule is in the last position you will sort very relevant results.
- 💡 The names of non-existent ranking rules are not taken into account. Thus, it does not return an error on a message request if the user sends a misnamed ranking rule.
...
- Support computationak functions avg, sum, min, max, median. A workaround is to pre-compute values in the document before indexing and sort them at search time with a `sort` on the pre-computed field. e.g. sort="precomputed_avg_price:desc".
