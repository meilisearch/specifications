...
| UserX   | UserX represent any end-user making a search from the frontend interfaces provided by the company who Anna and Mark work for. |
...
Only the master-key allows to manage the API keys.
...
| actions | array   | A list of actions permitted for the API key. `["*"]` for all actions. See `Actions List Definition` part. |
| indexes  | array  | A list of indexes permitted for the API key. `["*"]` for all indexes. The `*` character can be used as a wildcard. E.g. `"products_*"` to authorize access on all indexes starting with `products_`. |
| expiresInSeconds | integer | A number of seconds until the API Key will expire. `null` equals no expiration time. |
| description | string  | A description for the API key. `null` if empty. |
...
| updatedAt | string | Represent the date and time as `ISO-8601` format when the API key has been updated. **Generated by MeiliSearch**. `null` if never updated. |
...
| expiresInSeconds | integer | Required | A number of seconds until the API Key will be valid. `null` equals to having no expiration time. **Default**: `No Default` |
| indexes  | array  |  Optional | `[*]` for all indexes. `*` character can be used as a wildcard. E.g. `products_*` to authorize access on all indexes starting with `products_`. **Default**: `["*"]` - All indexes |
| description | string  | Optional | A description for the API key. **Default**: `null` |

#### Actions List Definition

> `:authorizedIndexes` can be the possible value for the `indexes` field of an `API key` resource.
...
```json
{
    ...
    "createdAt": "2021-11-12T10:00:00.000000Z"
}
```
...
- If set, `description` should be a string.
- If set, `indexes` should be an array of string.
- If set, `expiresInSeconds` must be a positive integer or null.
...
```json
{
    ...
    "expiresInSeconds": 3550, //50 elapsed seconds since the API key creation request. MeiliSearch computes it.
    "createdAt": "2021-11-12T10:00:00.000000Z",
    "updatedAt": "2021-11-12T10:00:00.000000Z"
}
```
...
```json
{
    "description": "Manage Document API key",
    "indexes": ["products_*"], //Has now access to all indexes starting with `products_`
    "actions": [
        "documents.*" //Has now access to all operations on the documents API endpoint
    ],
    "expiresInSeconds": null //Becomes unlimited
}
```

> ⚠️ Any omitted fields are restored to his default value. See `POST`/`PUT` - `/keys` - API Key object payload definition part.
...
{
    "key": "d0552b41536279a0ad88bd595327b96f01176a60c2243e906c52ac02375f9bc4",
    "updatedAt": "2021-08-14T10:00:00.000000Z"
}
```
...
- `404 API Key Not Found`.
- `actions` field is mandatory and must be an array of `actions`.
- If set, `description` must be a string.
- If set, `indexes` must be an array of strings.
- If set, `expiresInSeconds` must be a positive integer or null.
    ...
    "results": [
        {
            ...
            "expiresInSeconds": 3550,
            "createdAt": "2021-08-13T10:00:00.000000Z",
            "updatedAt": "2021-08-13T10:00:00.000000Z"
        },
        {
...
> Expired `API keys` cannot be found on the `/keys` endpoints. They are automatically deleted by MeiliSearch.

> 👉 Note the two default generated API keys. When a `master` key is set at MeiliSearch first-launch, MeiliSearch generate two pre-configured `API Keys`. A `Default Search API Key` restricted to the `search` action and a `Default Admin API Key` to handle all operations (except managing API Keys) on a MeiliSearch instance.
...
### **As `Mark 👨🏻`, I am using a valid API Key but permission is not sufficient to access the requested API resource**
