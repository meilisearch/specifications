...
> ⚠️ Any omitted fields are restored to his default value. See `POST`/`PUT` - `/keys` - API Key object payload definition part. Only `description` field is concerned so far.
...
> 👉 Note the two default generated API keys. When a master key is set at MeiliSearch first-launch, we can generates two pre-configured `Stored API Keys`. A Default Search API Key restricted to the search action and a Default Admin API Key to handle all operations (except managing API Keys) on MeiliSearch.
