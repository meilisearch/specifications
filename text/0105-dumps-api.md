...
- If MeiliSearch is restarted after a dump creation, the dump's status will not appear on the `GET - /dumps/:uid` endpoint.
...
A unique identifier for the dump. Generated by Meilisearch.
...
The current `status` for the dump. The possible values are `processing` and `succeeded`.
...
### 3.3. Dumps API Definition

#### 3.3.1. POST `/dumps`

Create a dump

##### 3.3.1.1. Body Payload
N/A
...
#### 3.4.3. `--ignore-dump-if-db-exists`
