...
It is not possible to fill it in when creating or updating an index.
...
The creation date time when the index has been created.
...
The last update date time when the index has been updated.
...
The `name` field takes the value of given `uid`.
